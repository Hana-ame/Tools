...
function loadUserModels() {
    try {
        const saved = localStorage.getItem('aichat_models');
        if (saved) userModels = JSON.parse(saved);
    } catch {}
}

function saveUserModels() {
    localStorage.setItem('aichat_models', JSON.stringify(userModels));
}

function populateModelSelect() {
    const sel = document.getElementById('modelSelect');
    const current = config.model || 'deepseek-v4-flash-free';
    const allModels = [...DEFAULT_MODELS, ...userModels.filter(m => !DEFAULT_MODELS.includes(m))];
    sel.innerHTML = allModels.map(m => `<option value="${m}" ${m === current ? 'selected' : ''}>${m}</option>`).join('');
}
